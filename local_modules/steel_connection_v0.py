...
class FilletWeld(BaseWeld):
    ...
    def getWeldDesignStrength(self,Fpar,Fperp):
        '''Return the weld design strength according
        ...
        Awe=self.length*self.weldThroat
        Rn_weld=Fnw*Awe
        return Rn_weld 
        
    def getCF_AISCverif(self,baseMetal):
        '''Return the capacity factor of the weld
        according to section J.2 of AISC (reference [2])
        '''
        # force acting on the weld
        (Fpar,Fperp)=self.getTotalIntForc()
        #Base metal design strength  (Sect. J2 AISC)
        A_BM=self.length*self.weldSz
        F_nBM=0.75*baseMetal.fu   #sect. J4-2 AISC)
        Rn_base=F_nBM*A_BM
        #Weld  design strengths  (Sect. J2 AISC)
        Rn_weld_V=self.getWeldDesignStrength(Fpar,Fperp)
        #design shear internal forces
        V=math.sqrt(Fpar**2+Fperp**2)
        # Capacity factors
        Rn=min(Rn_base,0.75*Rn_weld_V)
        CF=V/Rn
        return CF
...
class MultiFilletWeld(object):
    ...
    def getLstCF_AISCverif(self,baseMetal):
        '''Return the list of capacity factors of the welds
        according to section J.2 of AISC (reference [2])
        '''
        lstCF=list()
        for w in self.lstWelds:
            lstCF.append(w.getCF_AISCverif(baseMetal))
        return lstCF

    def getMaxCF_AISCverif(self,baseMetal):
        '''Return the maximum capacity factors of the welds
        according to section J.2 of AISC (reference [2])
        '''
        lstCF=self.getLstCF_AISCverif(baseMetal)
        return max(lstCF)
    

# Functions to check bolts and welds according to AISC-16
    
def aisc_check_bolts_welds(modelSpace,ULSs,boltSets2Check=[],welds2Check=[],baseMetal=None,meanShearProc=True,resFile=None):
    ...
    for ULS in ULSs:
        ...
        #welds checking
        set_welds_check_resprop_current_LC(ULS,singlWelds,baseMetal)
    #print results of bolt checking
    if len(boltSets2Check)>0:
        Lres=scr.print_bolt_results(boltSets2Check)
        if resFile:
    ...
    if len(welds2Check)>0:
        Lres=scr.print_welds_results(singlWelds)
        if resFile:
...
def init_prop_checking_welds(welds2Check):
    ...
    for checkW in welds2Check:
        LS='' ; Fpar=0 ; Fperp=0 ;  Rd=0 ; CF=0
        if not hasattr(checkW,'lstWelds'):
            singlWelds.append([checkW,[CF]])
        else:
            for w in checkW.lstWelds: singlWelds.append([w,[LS,Fpar,Fperp,Rd,CF]])
    return singlWelds
...
def set_welds_check_resprop_current_LC(ULS,singlWelds,baseMetal):
    for i in range(len(singlWelds)):
        weld=singlWelds[i][0]
        CFinit=singlWelds[i][1][-1]
        CF=weld.getCF_AISCverif(baseMetal)
        if CF>CFinit:
            LS=ULS
            (Fpar,Fperp)=weld.getTotalIntForc()
            Rd=0.75*weld.getWeldDesignStrength(Fpar,Fperp)
            singlWelds[i][1]=[LS,Fpar,Fperp,Rd,CF]
...
def gen_bolts_xc_conn_model(modelSpace,matchedBolts):
    ...
    for blt in matchedBolts:
        setName=blt.getSetName()
        if setName not in boltSets2Check.keys():
            D=blt.getDiameter()
            mat=blt.getMaterial()
            boltSets2Check[setName]={'boltSet': None, 'boltCheckTyp': None, 'boltSeed': None}
            boltSets2Check[setName]['boltSet']=preprocessor.getSets.defSet(setName)
            boltSets2Check[setName]['boltSet'].description=blt.getSetDescription()
            boltSets2Check[setName]['boltCheckTyp']=ASTM_materials.BoltFastener(diameter=D, steelType=mat)
            boltSets2Check[setName]['boltSeed']=Bolt(diam=D,mat=mat)
        bltSeed=boltSets2Check[setName]['boltSeed']
        pntA=blt.endA.kPoint
        pntB=blt.endB.kPoint
        bltSeed.createBolt([pntA,pntB],setName)  #bolt generation and addition to set
        #raddi end A
        if blt.endA.hole:
